...
def generar_texto_secuencial(prompt, modelo, tokenizer, longitud=50):
    secuencia = tokenizer.texts_to_sequences([prompt])[0]
    texto_generado = prompt

    for _ in range(longitud):
        entrada = pad_sequences([secuencia], maxlen=MAXLEN, padding='pre')
        pred = modelo.predict(entrada, verbose=0)
        
        # Suponiendo que es un modelo secuencial que predice próximo token
        siguiente_token = np.argmax(pred[0])

        if siguiente_token == 0:
            break  # Token desconocido o final

        palabra = tokenizer.index_word.get(siguiente_token, "")
        if palabra == "":
            break
